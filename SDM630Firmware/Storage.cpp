...
bool isChanged() {
  ...
  if( changed ) {
#ifdef Debug
  Serial.println(F("Storage: changes detected"));
#endif
    storageMakeSnapshot();
    changedOn = millis();
  }
...
void storageRead() {
  ...
  if( (storageSnapshot[0] != 0x41) || (storageSnapshot[1] != 0x45) ) {
    Serial.println(F("Storage reading error. Resetting"));
    memset( storageSnapshot, 0, STORAGE_Size );
...
void storageSave() {
  storageInit();
  if( isChanged() ) {
    Serial.println(F("Writing Storage"));
...
void storageReset() {
  storageInit(true);  
  Serial.println(F("Clearing Storage"));
  memset( storageSnapshot, 0, sizeof(storageSnapshot) );
  delay(1000);;
  ESP.restart();
}
